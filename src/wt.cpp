...
IntervalSymbolsResults FN(wt_interval_symbols, WT_ID)
        (WT *x, wt_size_type start_index, wt_size_type end_index) {
    ...
    results._symbols_in_range = (wt_value_type *) malloc(results.num_symbols_in_range
                                                         * sizeof(wt_value_type));
    for (auto i = 0; i < results.num_symbols_in_range; ++i)
        results._symbols_in_range[i] = cs[i];

    results._rank_start_index = (wt_size_type *) malloc(results.num_symbols_in_range
                                                        * sizeof(wt_size_type));
    for (auto i = 0; i < results.num_symbols_in_range; ++i)
        results._rank_start_index[i] = rank_c_i[i];

    results._rank_end_index = (wt_size_type *) malloc(results.num_symbols_in_range
                                                      * sizeof(wt_size_type));
    for (auto i = 0; i < results.num_symbols_in_range; ++i)
